...
using Surging.Cloud.Caching.RedisCache;
...
namespace Surging.Cloud.Caching.AddressResolvers.Implementation
{
    /// <summary>
    /// 默认地址解析程序
    /// </summary>
...
new ConcurrentDictionary<string, ServiceCache>();
        ...
        public async Task<ConsistentHashNode> Resolver(string cacheId, string item)
        {
            ...
            if (_logger.IsEnabled(LogLevel.Debug))
                _logger.LogDebug($"根据缓存id：{cacheId}，找到以下可用地址：{string.Join(",", address.Select(i => i.ToString()))}。");
